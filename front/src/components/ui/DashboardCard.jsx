...
function DashboardCard({ tierFolderData, cardValue, setCardValue }) {
  if(tierFolderData === 'creating'){
  console.log(true)
}
console.log(tierFolderData)
  ...
  return (
    <Link to={  tierFolderData !== 'creating' ? `tier-list/${id}` : ""  }>
    <div
      className="
    ...
    >
      <div className="relative flex-1 w-full bg-[#f9f9fa] overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-black/0 via-transparent to-transparent opacity-70" />
      ...
      {
        tierFolderData !== 'creating' ? <h3 className="text-gray-800 font-semibold text-base line-clamp-1 tracking-tight group-hover:text-black transition-colors">
          {title || ""}
        </h3>
        : <input type="text" value={cardValue} name="" id="" />
      }
