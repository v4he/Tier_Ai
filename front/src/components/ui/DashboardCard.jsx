...
function DashboardCard({ tierFolderData, setTierFolder }) {
  ...
  return (
    <div className="group relative w-full aspect-square">
      <Link to={`tier-list/${id}`} className="block w-full h-full">
        <div className="w-full h-full bg-white rounded-[30px] shadow-[0_8px_30px_rgba(0,0,0,0.03)] hover:shadow-[0_15px_40px_rgba(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300 ease-out cursor-pointer flex flex-col overflow-hidden">
      ...
      <div
        ...
        role="button"
        tabIndex={0}
        aria-label="Supprimer cette tier list"
        className="absolute top-4 right-4 p-2 rounded-xl bg-white text-black shadow-md opacity-0 translate-x-2 -translate-y-1 group-hover:opacity-100 group-hover:translate-x-0 group-hover:translate-y-0 hover:bg-black hover:text-white transition-all duration-300 ease-out z-10 cursor-pointer"
      >
